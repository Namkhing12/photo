  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full flex-grow flex flex-col items-center justify-center p-4 md:p-8">
        ...
        <div 
          className="bg-white/60 backdrop-blur-xl rounded-[2rem] md:rounded-[3rem] shadow-2xl border border-white/50 p-6 md:p-8 w-full max-w-5xl flex flex-col md:flex-row gap-8 items-start justify-center transition-all"
          ...
          <div className="flex-1 w-full flex flex-col items-center">
            ...
            <div className="md:hidden text-center mb-4">
               <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2 justify-center">
                 <Sparkles className="text-pink-500" /> Photo Booth
               </h1>
            </div>
            ...
            <div className="flex flex-wrap gap-3 mb-6 justify-center w-full z-50">
               <div className="relative">
                  <button 
                    ...
                    className="flex items-center gap-2 bg-white px-4 py-2.5 rounded-xl border border-gray-200 text-gray-700 text-sm font-bold hover:border-pink-300 transition-all shadow-sm min-w-[130px] justify-between"
                  >
                    ...
                    <div className="absolute top-full left-0 mt-2 w-[280px] bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden p-2 grid grid-cols-2 gap-1 z-50 animate-in fade-in zoom-in-95">
                      {frameOptions.map((option) => (
                        <button
                          ...
                          className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors text-left ${selectedFrame.id === option.id ? 'bg-gray-100 font-bold text-gray-900' : 'hover:bg-gray-50 text-gray-600'}`}
                        >
               ...
               <div className="relative">
                  <button 
                    ...
                    className="flex items-center gap-2 bg-white px-4 py-2.5 rounded-xl border border-gray-200 text-gray-700 text-sm font-bold hover:border-pink-300 transition-all shadow-sm min-w-[120px] justify-between"
                  >
                    ...
                    <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden p-1 z-50">
                      {timerOptions.map((time) => (
                        <button
                          ...
                          className={`flex justify-between w-full px-3 py-2 text-sm rounded-lg ${timerSetting === time ? 'bg-pink-50 text-pink-500 font-bold' : 'hover:bg-gray-50 text-gray-600'}`}
                        >
               ...
               <button 
                 ...
                 className="flex items-center gap-2 bg-white border border-pink-200 px-4 py-2.5 rounded-xl text-pink-500 text-sm font-bold hover:bg-pink-50 shadow-sm transition-all"
               >
                 <Upload size={16} />
                 <span>อัปโหลด</span>
               </button>
            ...
            {/* Camera Area */}
            <div className="relative w-full max-w-[420px] aspect-[3/4] bg-white rounded-[2rem] p-3 shadow-2xl border border-gray-100 mb-6 group">
               <div className="relative w-full h-full rounded-[1.5rem] overflow-hidden bg-gray-900">
                  ...
                  <button 
                    ...
                    className="absolute top-4 right-4 bg-black/40 backdrop-blur-md text-white p-3 rounded-full hover:bg-pink-500 transition-all z-40 active:scale-90 border border-white/20"
               ...
               <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-2 z-20">
                 {Array.from({ length: selectedFrame.count }).map((_, i) => (
                   <div 
                     key={i} 
                     className={`w-2.5 h-2.5 rounded-full transition-all ${i < photos.length ? 'bg-pink-500 scale-110' : 'bg-white/50'}`}
                   ></div>
                 ))}
               </div>
            </div>
                ...
                {filters.map((filter) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative px-5 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all border ${isActive ? 'bg-pink-50 border-pink-400 text-pink-500' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                    >
                      {filter.name}
                    </button>
                  );
            ...
            <div className="mt-4 flex flex-col items-center gap-3 w-full">
              {!isComplete ? (
                <button 
                  ...
                  className="bg-gradient-to-r from-[#FF5C8D] to-[#FF8FAB] text-white text-lg font-bold px-12 py-4 rounded-full shadow-lg shadow-pink-200 hover:scale-105 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-70"
                >
                  <Camera size={24} />
                ...
                <div className="flex flex-col md:flex-row gap-4 items-center w-full justify-center">
                    <button 
                      ...
                      className="bg-gray-100 text-gray-600 px-6 py-3 rounded-full font-bold hover:bg-gray-200 transition-all flex items-center gap-2 shadow-sm"
                    >
                      <RefreshCw size={18} /> ถ่ายภาพล่าสุดใหม่
                    </button>

                    <button 
                      onClick={() => onFinish?.(photos, activeFilterClass)}
                      className="bg-gradient-to-r from-[#FF5C8D] to-[#FF8FAB] text-white text-lg font-bold px-12 py-3 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center gap-3 animate-pulse"
                    >
                      <span>ต่อไป</span>
                      <ArrowRight size={24} />
                    </button>
        ...
        <div className="mt-8 text-pink-300/80 text-xs font-medium text-center">
